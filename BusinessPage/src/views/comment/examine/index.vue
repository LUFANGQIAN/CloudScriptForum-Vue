    ...
    <div class="card">
      ...
      <div class="card-second">
        ...
        <div class="auto-audit">
          <span class="label">评论自动审核</span>
          <el-switch v-model="autoAuditEnabled" :loading="autoAuditLoading" @change="onToggleAutoAudit" />
        </div>
      </div>
...
import { adminGetCommentList, adminDeleteComment, adminDeleteBatchComment, adminExamineComment, adminExamineBatchComment, adminSearchComment, adminGetCommentAutoAudit, adminSetCommentAutoAudit } from "@/api/comment";
...
const fetchAutoAudit = async () => {
  autoAuditLoading.value = true;
  try {
    const res = await adminGetCommentAutoAudit();
    autoAuditEnabled.value = !!res.data.data;
  } finally {
    autoAuditLoading.value = false;
  }
};

const onToggleAutoAudit = async (val) => {
  autoAuditLoading.value = true;
  try {
    await adminSetCommentAutoAudit(val);
    ElMessage.success(val ? "已开启自动审核" : "已关闭自动审核");
  } catch (e) {
    autoAuditEnabled.value = !val;
